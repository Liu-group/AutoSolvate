...
class IonForceFieldBuilder():
    ...
    def get_mass(self):
        with open(self.xyzfile, 'r') as f:
            ...
            if matched_key:
                ...
                try:
                    atomnumber = METAL_PDB[matched_key][1]
                    mass = METAL_PDB[matched_key][2]
    ...
    def packSLUSLV(self):
        ...
        if self.solvent not in  custom_solv_dict.keys():
            ...
            shutil.copy(solvent_pdb_origin, solvent_pdb)
    ...
    def writeTleapcmd_add_solvent_user(self):
        ...
        if self.charge != 0:
            if self.charge > 0:
                ...
                ion = 'Na+'
            ofile.write("addIons2 mol " + ion + " " + str(abs(self.charge)) + "\n")
        ofile.write("check mol\n")
        ofile.write('savepdb mol '+self.outputFile+'.pdb\n')
        ofile.write("saveamberparm mol " + self.outputFile + ".prmtop " + self.outputFile + ".inpcrd\n")
        ofile.write("quit\n")
