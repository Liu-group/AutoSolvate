...
class TleapDocker(GeneralDocker):
    ...
    def _resolve_custom_tleap_path(self, mol:TransitionMetalComplex):
        ...
        if env_tleap_path is not None:
            ...
            return
        # 3. not found
        self.logger.warning("Custom tleap path not set in TransitionMetalComplex object or environment variable.")
        self.logger.warning("Proceeding with default tleap executable, but bond adding issue may persist.")

    def check_system_transitionmetalcomplex(self, mol:TransitionMetalComplex):
        self.logger.info("Checking system {:s}...".format(mol.name))
        self.logger.info("This is a transition metal complex.")
        self.logger.info("Metal center: {}".format(mol.metal_residue_names))
        self.logger.info("Number of metal-ligand bonds: {}".format(mol.n_metal_legand_bonds))
        if mol.n_metal_legand_bonds > 8:
            self._resolve_custom_tleap_path(mol)
    ...
    def check_system_solvatedsystem(self, mol:SolvatedSystem):
        ...
        for slu in mol.solutes:
            ...
            elif isinstance(slu, TransitionMetalComplex):
                self.check_system_transitionmetalcomplex(slu)
    ...
    def check_system(self, mol:System):
        if isinstance(mol, Molecule):
            ...
            self.check_system_transitionmetalcomplex(mol)
        else:
    ...
    def generate_cmd(self, mol:System) -> str:
        if not self.use_custom_tleap:
            ...
            cmd = '{} -s -f {} > {}'.format(self.custom_tleap_path, self.leapinp, self.leapout)
        self.logger.info("Tleap command: {}".format(cmd))
        return cmd
