...
class AutoMCPB():
    ...
    """
    def __init__(self, xyzfile='', filename = None, metal_charge=None, metal_ID='',chargefile=None,mode='A',spinmult=None,round=None,software = 'g09',ECP='Y'): #liglist='', denticity='',ligcons='',atomsinfo='',ligand_charge='',metal_name=''):
    ...
    def inputCheck(self):
        ...
        try:
            value = int(self.spinmult)
            print("Valid integer for spins!")
        except ValueError:
            print("The provided spinmult is not an integer.")
            sys.exit()
        try:
            value = int(self.metal_charge)
            print("Valid integer for metal charge!")
        except ValueError:
            print("The provided metal charge is not an integer.")
    ...
    def resonance_check(self, ligandsdf,ligandmetalsdf,ligandmetalxyz):

        cmd = 'obabel ' + ligandmetalxyz + ' -O ' + ligandmetalsdf
        with open(ligandmetalsdf.split('.sdf')[0]+'_sdf.log', 'w') as f:
            subprocess.call(cmd, shell=True, stdout=f, stderr=subprocess.STDOUT)
        
        ligand_metal_mol = mol3D()
        ligand_metal_mol.readfromxyz(ligandmetalxyz)

        ligand_metal_breakdown_infos = ligand_breakdown(ligand_metal_mol)
        ligand_metal_linker =  ligand_metal_breakdown_infos[2][0]

        #print(ligand_metal_linker)
        heavyatoms_ligand_metal_mol = []
        allatoms_ligand_metal_mol = []

        with open(ligandmetalxyz,'r') as f:
            data = f.readlines()
            for ID, line in enumerate(data[2:]):
                ...
                y = float(line.split()[2])
                z = float(line.split()[3])
                allatoms_ligand_metal_mol.append(AtomInfo(atomtype=atomtype,coordinate=[x,y,z],atomnum=ID))
                if atomtype not in ['H']:
                    heavyatoms_ligand_metal_mol.append(AtomInfo(atomtype=atomtype,coordinate=[x,y,z],atomnum=ID))
        
        metalID = allatoms_ligand_metal_mol[-1].atomnum

        suppl_ligand = Chem.SDMolSupplier(ligandsdf)
        ligand_radicals = {}
        mol_ligand = [mol_ligand for mol_ligand in suppl_ligand][0]
      #  print('#####',mol_ligand.GetAtoms())
        for newID, atom in enumerate(mol_ligand.GetAtoms()):
            radical = atom.GetNumRadicalElectrons()
            ID = heavyatoms_ligand_metal_mol[newID].atomnum
            ligand_radicals[ID]  = radical
        linked_ligand_radical = {}
        
        for atom in ligand_metal_linker:
            linked_ligand_radical[atom] = ligand_radicals[atom] 
        
        extra_atomcase = []
        for atom in ligand_radicals:
            if ligand_radicals[atom] > 0:
                if atom not in  ligand_metal_linker:
                    extra_atomcase.append(atom)

    #  print(extra_atomcase)

        with open(ligandmetalsdf,'r') as f:
            linker_dic = {}
            data = f.readlines()
            atomnumbers = int(data[3].split()[0])
            for i in range(atomnumbers):
        ...
        longpair_of_linked_atom = {}
        new_radicals = {}

        for atom in range(len(allatoms_ligand_metal_mol))[:-1]:
            atomtype = allatoms_ligand_metal_mol[atom].atomtype
            valence_electron = valence_electrons[str.upper(atomtype)]
            longpair = valence_electrons_full[str.upper(atomtype)] - (int(valence_electron) +  linker_dic[atom])
            new_radicals[atom] = longpair
         #   print(atom,longpair)
            if atom in ligand_metal_linker:
                longpair_of_linked_atom[atom] = longpair
    # print(longpair_of_linked_atom)
        
        new_extra_charge = 0
        for atom in extra_atomcase:
            new_extra_charge = new_extra_charge + new_radicals[atom]
        
        extra_radical = 'Y'
        if new_extra_charge == 0:
            if longpair_of_linked_atom != linked_ligand_radical:
                extra_radical = 'YN'
   #     print('--------=====',longpair_of_linked_atom,linked_ligand_radical)
        return new_radicals,extra_radical
                ...
                for atom in ligand_metal_breakdown_infos[2]:
                    for ID in atoms_with_radical:
                        if ID in atom:
                            ...
                            print('#######start resonance check')
                            charge_of_ligand = charge_of_ligand + atoms_with_radical[ID]*(-1)
                            ligandmetalxyz = sdf + '_' + self.metal_name + '.xyz'
                            ligandsdf = sdf + '.sdf'
                            ligandmetalsdf = sdf + '_' + self.metal_name + '.sdf'
                            new_radicals, extra_charge  = self.resonance_check(ligandsdf,ligandmetalsdf,ligandmetalxyz)
                        #   print('-----',ligandsdf, new_radicals,extra_charge)
                            if extra_charge == 'Y':
                                print('Warning: atom',ID, 'have extra charges, please use option --mode M to set the charge of each ligand in charge.in')
    ...
    def coorination_or_covalent(self, ligandname, complex_xyz, ligand_metal_xyz):
        connections = []
        atom_number_dic = self.find_atomnumber(complex_xyz, ligand_metal_xyz)
        sdffile = ligandname + '.sdf'
        suppl = Chem.SDMolSupplier(sdffile)
        mol = [mol for mol in suppl][0]
        heavy_atoms = []
        new_complex_mol = mol3D()
        new_complex_mol.readfromxyz(complex_xyz)

        with open(ligandname+'.xyz') as f:
            data = f.readlines()
            for ID, line in enumerate(data[2:]):
                atomtype = line.split()[0]
                x = float(line.split()[1])
                ...
                z = float(line.split()[3])
                if atomtype not in ['H']:
                    heavy_atoms.append(AtomInfo(atomtype=atomtype,coordinate=[x,y,z],atomnum=ID))

        charge,extra_charge = self.check_radical(ligandname +'.xyz')
        new_charges,new_extra_charge = self.resonance_check(ligandsdf = ligandname + '.sdf',ligandmetalsdf=ligandname +'_' + self.metal_name + '.sdf',ligandmetalxyz=ligand_metal_xyz)
      #  print(new_charges)
        for newID, atom in enumerate(mol.GetAtoms()):
            radical = atom.GetNumRadicalElectrons()
            atomID_in_ligand = heavy_atoms[newID].atomnum
            atomID_in_complex = atom_number_dic[atomID_in_ligand]
            pairs = ligand_breakdown(new_complex_mol)[2]
            all_pairs = [linked_atom for sublist in pairs for linked_atom in sublist]
            if extra_charge == 'N':
                if atomID_in_complex in all_pairs:
                    radical = atom.GetNumRadicalElectrons()
                    if radical > 0:
                        connections.append('atom ' + str(atomID_in_complex +1) + '   ' + str(radical) + ' covalent bond')
                    elif radical == 0:
                        connections.append('atom ' + str(atomID_in_complex + 1) + '   ' + str(1) + ' coordination bond')
            elif extra_charge == 'YN':
                if atomID_in_complex in all_pairs:
                    radical = abs(new_charges[atomID_in_ligand])
                    if radical > 0:
                        connections.append('atom ' + str(atomID_in_complex +1) + '   ' + str(radical) + ' covalent bond')
                    elif radical == 0:
                        connections.append('atom ' + str(atomID_in_complex + 1) + '   ' + str(1) + ' coordination bond')
            elif extra_charge == 'Y':
                if atomID_in_complex not in all_pairs:
                    if radical > 0:
                        connections.append('atom ' + str(atomID_in_complex + 1) + '   ' + str(radical) + ' radical not linked with metal')
                else:
                    if radical > 0:
                        connections.append('atom ' + str(atomID_in_complex +1) + '   ' + str(radical) + ' covalent bond')
                    elif radical == 0:
    ...
    def run_MCPB_1(self):
        if self.round in ['1']:
            ...
            if self.filename + '_large_mk.com' in glob('*.com'):
                print(self.filename + '_large_mk.inp', 'is generated for GAMESS input')
            elif self.filename + '_small_opt.com' in glob('*.com'):
                print(self.filename + '_small_opt.inp', 'is generated for GAMESS input')
            elif self.filename + '_small_fc.com' in glob('*.com'):
                print(self.filename + '_small_fc.inp', 'is generated for GAMESS input')
            elif self.filename + '_small_opt.inp' in glob('*.com'):
                print(self.filename + '_small_opt.com', 'is generated for GAUSSIAN input')
            elif self.filename + '_small_fc.com' in glob('*.com'):
                print(self.filename + '_small_fc.com', 'is generated for GAUSSIAN input')
            elif self.filename + '_large_mk.inp' in glob('*.com'):
                print(self.filename + '_large_mk.com', 'is generated for GAUSSIAN input')
    ...
    def run_MCPB_2(self):    
        if  self.round in ['2','2b','2e','2s','2z']:
            ...
            if self.software in ['g09','g16']:
                for resultfile in [self.filename + '_small_fc.log',self.filename + '_small_opt.chk',self.filename + '_small_opt.fchk',
                                    self.filename + '_small_opt.log',self.filename + '_small_opt.log', self.filename + '_large_mk.chk',
                                    self.filename + '_large_mk.esp', self.filename + '_large_mk.log']:
                    if resultfile in glob('*'):
                ...
                for resultfile in [self.filename + '_small_fc.orcaout',self.filename + '_small_opt.orca'+'_trj.xyz',
                                   self.filename + '_small_opt.orcaout',self.filename + '_large_mk.log']:
                    if resultfile in glob('*'):
                        print(resultfile  + ' is generated by the first round of MCPB.py \n' )
                    else:
                        print('Error', resultfile  + ' is not generated by the first round of MCPB.py \n' )
