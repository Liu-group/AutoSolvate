...
#        subprocess.call(cmd, shell=True)
    ...
    def getFrcmod(self):
        cmd1 ="$AMBERHOME/bin/antechamber -i solute.xyz.pdb -fi pdb -o solute.mol2 -fo mol2 -c bcc -s 2 -nc "+str(self.slu_netcharge) + " -m " + str(self.slu_spinmult)
        print(cmd1)
        cmd2 ="$AMBERHOME/bin/parmchk2 -i solute.mol2 -f mol2 -o solute.frcmod"
    ...
    def processPackmolPDB(self):
        data=open('ch3cn_solvated.pdb').readlines()
        output=open('processed-ch3cn_solvated.pdb','w')
        this_resid = 1
        last_resid = 1
        for line in data:
            if 'ATOM' in line:
    ...
    def packSLUSLV(self):
        if self.solvent != 'acetonitrile':
            ...
            packmol_inp.write("\n")
            packmol_inp.write("output ch3cn_solvated.pdb\n")
            packmol_inp.write("\n")
    ...
    def writeTleapcmd_ch3cn_solvated(self):
        ...
        f.write("\n")
        f.write("SYS = loadpdb processed-ch3cn_solvated.pdb\n")
        f.write("check SYS\n")
        f.write("\n")
        ...
        f.write("savepdb SYS ch3cn_solvated.leap.pdb\n")
        f.write("quit\n")
        f.close()
...
if __name__ == '__main__':
    builder = solventBoxBuilder('coumarin7oh_min.xyz', 'water', slu_netcharge=1, slu_spinmult=2)
