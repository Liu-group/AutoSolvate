...
class AutoMCPBDocker(GeneralDocker):
    ...
    def process_output(self, mol: TransitionMetalComplex) -> None:
        ...
        if os.path.exists(os.path.join(self.mcpb_folder, self.mcpb_frcmod)):
            self.logger.info(f"Find the MCPB generated frcmod file {self.mcpb_frcmod}.")
            setattr(mol, "frcmod", os.path.join(self.mcpb_folder, self.mcpb_frcmod))
            mol.mcpb_frcmod_exists = True
        else:
            self.logger.warning(f"The MCPB generated frcmod file {self.mcpb_frcmod} does not exist.")
            self.logger.warning(f"Usually we expect the frcmod file {self.mcpb_frcmod} to be generated by AutoMCPB.")
            self.logger.warning(f"Will generate the frcmod file from prmtop.")
            mol.mcpb_frcmod_exists = False

        mol.update()

        if str(mol.totalcharge).upper() in ["DEFAULT", "AUTO"]:
