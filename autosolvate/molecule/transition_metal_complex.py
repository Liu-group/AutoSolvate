...
class TransitionMetalComplex(System):
    ...
    def __init__(
        ...
        ----------
        xyzfile : str
            xyz or pdb file of the transition metal complex. 
        metal_charge : int
            charge of the center metal atom. Must be provided.
        multiplicity : int
            multiplicity of the transition metal complex.
        legand_charge_file : str, optional
            The file contains the charge of each legand. if not provided, the charge of each legand will be set to 0. Refer to autosolvate.FFmetalcomplex for the format. by default "".
        prmtop : str
        ...
        custom_tleap_path : str, optional
            Custom tleap path for generating prmtop and inpcrd files, by default "" will use the tleap in AmberTools.
        """
        # copilot is even more smart than me （本来挺喜欢你的结果你开始自以为是了）

        self.name           = process_system_name(name, xyzfile, support_input_format = ["xyz", "pdb"])
        self.folder         = os.path.abspath(folder)
        self.metal_charge   = metal_charge
        self.multiplicity   = multiplicity
        self.spinmult       = multiplicity
        self.charge_file    = legand_charge_file
        ...
        self.custom_tleap_path = custom_tleap_path
        self.n_metal_legand_bonds = 0

        self.mcpb_frcmod_exists = False
    ...
    def process_input_xyz(self, xyzfile:str) -> None:
        for e in self._SUPPORT_INPUT_FORMATS:
    ...
    def check_system(self):
        if not self.prmtop:
            logger.info("The Prmtop file is not generated yet. Will be generated by AutoMCPB.")
            logger.info("Please make sure AutoMCPB would run successfully.")
            logger.info("Ignoring the following checks.")
            return 
        ...
        self.n_metal_legand_bonds = len(self.metal_legand_bonds)
        self.logger.info(f"Number of metal-legand bonds: {self.n_metal_legand_bonds}")

        # regarding tleap's erroneous behavior when the number of metal-legand bonds is greater than 8
        # this is due to tleap does not recognize very high coordination number
        if self.n_metal_legand_bonds > 8:
            self.logger.warning(f"The number of metal-legand bonds is {self.n_metal_legand_bonds}, which is greater than 8.")
            self.logger.warning("This system is probably Ferrocene-like, Rhodium centered, or Lanthanide and Actinide centered complex.")
            self.logger.warning("The original tleap in AmberTools is known to missing bonds when the coordination number is very high.")
            self.logger.warning("In this case, using our custom compiled tleap is highly recommended to avoid tleap errors.")
        ...
        for metal_residue_name in self.metal_residue_names:
            if not os.path.exists(f"{self.origin_folder}/{metal_residue_name}.mol2"):
                ...
                raise FileNotFoundError(f"Metal residue {metal_residue_name}.mol2 does not exist.")
        ...
        for legand_residue_name in self.legand_residue_names:
            if not os.path.exists(f"{self.origin_folder}/{legand_residue_name}.mol2"):
                ...
                raise FileNotFoundError(f"Legand residue {legand_residue_name}.mol2 does not exist.")
        ...
        if not self.mcpb_frcmod_exists or not hasattr(self, "frcmod") or not isinstance(self.frcmod, str) or not os.path.exists(self.frcmod):
            self.logger.info(f"Creating frcmod file from prmtop: {self.prmtop}")
